...
db.exec('DELETE FROM auth_events; DELETE FROM messages; DELETE FROM transactions; DELETE FROM users;');
...
insertUser.run('alice', hash('password123'), 'alice@securebank.lab', 1500.0, 'Cliente demo A');
insertUser.run('bob', hash('password123'), 'bob@securebank.lab', 800.0, 'Cliente demo B');
insertUser.run('admin', hash('admin123'), 'admin@securebank.lab', 10000.0, 'Administrador demo');
...
console.log('Seed OK (segura). Usuarios: alice/password123, bob/password123, admin/admin123');
